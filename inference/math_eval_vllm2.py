...
# Robust boxed extractor (supports nested braces) — single source of truth
BOXED_OPEN = re.compile(r'\\boxed\*?\s*\{')
def _unbox_last(text: str) -> Optional[str]:
    if not text:
        return None
    last = None
...
_ANS_HINT = re.compile(
    r"(?is)^\s*answer\s*:\s*(.+)$|"  # line start
    r"\b(?:the\s+)?(?:final\s+)?answer\s*(?:is\s+equal\s+to|is|=|equals|:)\s*([^\n]+)"
)
_ASSIGN_HEAD = re.compile(r'^\s*[A-Za-z][A-Za-z0-9_]*(?:\([^)]*\))?\s*=\s*')
...
def _numericish_last(s: str) -> str:
    if not s:
        return ""
    ...
    s = _MOD_TAIL.sub("", s)
    s = _strip_wrappers(s)
    hits = list(DEC_SCI.finditer(s))
    if hits:
        return hits[-1].group(0)
    mf = list(MIXED_FRAC.finditer(s))
    if mf:
        return mf[-1].group(0)
    pf = list(PURE_FRAC.finditer(s))
    if pf:
        return pf[-1].group(0)
    return s

def _strip_prefixes_and_assign(s: str) -> str:
    if not s:
        return ""
    m = _ANS_HINT.search(s)
    if m:
        s = m.group(1) or m.group(2) or s
    s = _ASSIGN_HEAD.sub("", s)
    return s.strip()

def _extract_core(text: str) -> str:
    if not text:
        return ""
    s = _unbox_last(text) or text
    s = _strip_prefixes_and_assign(s)
    return _numericish_last(s)

def math_extract_pred(text: str) -> str:
    return _extract_core(text)
...
def _normalize_atomic(x: str) -> str:
    if x is None:
        return ""
...
def _latex_cleanup(expr: str) -> str:
    ...
    # allow pseudo-LaTeX sqrt{..} without backslash
    t = re.sub(r"(?<![A-Za-z\\])sqrt\{([^}]*)\}", r"sqrt(\1)", t)
    # implicit multiplication before function calls: 5sqrt(2) -> 5*sqrt(2), )sqrt(2) -> )*sqrt(2)
    t = re.sub(r"(\d|\))\s*([A-Za-z]+)\s*\(", r"\1*\2(", t)
    t = re.sub(r"\\\s+", " ", t)
    return t.strip()

def _sympy_parse(expr: str):  # -> Optional[sp.Expr]
    if expr is None:
        return None
    raw = expr.strip()
    if not raw:
        return None
    looks_latex = bool(re.search(r"\\[a-zA-Z]+", raw))  # only if backslash commands exist
    if looks_latex and parse_latex is not None:
        try:
            ...
            pass
    try:
        return sp.sympify(_latex_cleanup(raw), rational=True) if sp else None
    except Exception:
        return None
...
def grade_math_answer(pred: str, gold: str, atol: float = 1e-6) -> bool:
    ...
    Strategy:
      ...
      1) Containers: sets {..}, tuples (..,..), intervals.
      2) Scalar: normalized-string equality, numeric equality, sympy equality.
      3) Supports ± by branching.
    """
    ...
    if p0 in {"<NO_SOLUTION>", "<DNE>", "<INF>"} or g0 in {"<NO_SOLUTION>", "<DNE>", "<INF>"}:
        return p0 == g0
    # Containers (no set↔tuple mixing)
    ...
    # Scalars with ± expansion
    for pc in _expand_pm(p0):
        pn = _normalize_atomic(pc)
        for gc in _expand_pm(g0):
            gn = _normalize_atomic(gc)
            if pn == gn and pn != "":
                return True
            ...
            if p_num is not None and g_num is not None:
                if isinstance(p_num, Fraction) and isinstance(g_num, Fraction):
                    ...
                    try:
                        ...
                        pass
            if _sympy_equal(pn, gn) or _sympy_equal(pc, gc):
                return True
    return False
...
if __name__ == "__main__":
    ...
    incorr_path = "/home/leena/prm_shaping/analysis/incorr_math_vllm_0818.json"
